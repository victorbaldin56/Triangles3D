...
class TestGenerator {
 ...
 private:
  ...
  std::vector<Triangle_3> generateRandomTriangles(unsigned count) {
    ...
    std::transform(
        res.begin(), res.end(), res.begin(), [this, count](const auto& e) {
          // unfortunately, CGAL is unable to correctly handle degenerate
          // triangles in do_intersect()...
          auto&& t =
  ...
  Triangle_3 generateRandomTriangle(unsigned triangles_count) {
    ...
    auto&& trans_dist = std::uniform_int_distribution<int>(-triangles_per_edge,
                                                           triangles_per_edge);
    ...
    return Triangle_3(generateRandomPoint(tr_x, tr_y, tr_z),
  ...
  void writeAnswerToFile(const std::set<unsigned>& ans, unsigned test_num) {
    auto filepath = getOutputFileName(ans_directory_, "ans_", test_num, ".out");
    auto os = std::fstream();
    os.exceptions(std::ios::failbit);
    os.open(filepath, std::ios::trunc | std::ios::out);
  ...
  static std::string getOutputFileName(const fs::path& dir, const char* prefix,
                                       unsigned test_num,
 ...
 private:
  static constexpr unsigned kNumTests = 10;
  static constexpr auto kInputDir = "input";
  static constexpr auto kAnsDir = "ans";
  static constexpr unsigned kMinTriangles = 1;
  static constexpr unsigned kMaxTriangles = 1000;
